...
import com.breaktome.game_sample.events.IUpdateable;
import com.breaktome.game_sample.world.WorldState;
import com.breaktome.game_sample.world.utils.Directions;
...
public class World implements IArea, IPersist {
    ...
    public Region getRegion(int regionX, int regionZ) throws Exception {
        HashMap<Integer, Region> zMap = map.get(regionX);
        if(zMap == null)
            return null;
        return zMap.get(regionZ);
    }
    ...
    public Chunk getContainingChunk(int blockX, int blockZ) throws Exception {
        ...
        Region region = getContainingRegion(blockX, blockZ);
        if(region == null)
        {
            return null;
        }
...
//    }
    ...
    public Set<Chunk> getChunksInCircularRange(int blockX, int blockZ, int blockRadius) throws Exception
    {
        Set<Chunk> chunks = new HashSet<>();
        int chunkCount = (blockRadius / Chunk.size) + 1;
        int c = (int)Math.pow((blockRadius / (float)Chunk.size), 2);
        for(int i = (chunkCount*-1); i <= chunkCount; i++)
        {
            int a = (int)Math.pow(Math.abs(i), 2);
            for(int j = (chunkCount*-1); j <= chunkCount; j++)
            {
                int b = (int)Math.pow(Math.abs(j), 2);
                int xOffset = i * Chunk.size;
                int zOffset = j * Chunk.size;

                if((a+b) <= c)
                {
                    Chunk chunk = getContainingChunk(blockX + xOffset, blockZ + zOffset);
                    if(chunk != null)
                    {
                        chunks.add(chunk);
                    }
